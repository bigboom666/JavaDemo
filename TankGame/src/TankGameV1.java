...
class Tank extends JPanel implements KeyListener, Runnable {
    ...
    private Vector<Tanks> allTanks = new Vector<Tanks>();              //定义所有坦克集合
    // 定义敌人坦克数量
    ...
    public Tank() {
        ...
        myReward = new Reward(330, 230);
 /*       Thread tReward = new Thread(myReward);
        tReward.start();*/
        ...
        allTanks.addAll(eneMyTank);
        allTanks.add(myTank);


        //给  每个  EneMyTank的Vector<Tanks> allTank传入所有坦克信息，以便检查重叠
        for (EneMyTank tempTanks : eneMyTank) {
            tempTanks.setAllTank(allTanks);
        }

        //给奖励  所有坦克信息  查询重叠
        myReward.getAllTank(allTanks);
        ...
        try {
            ...
            imageStar = ImageIO.read(new File("star.png"));
        } catch (Exception e) {
            e.printStackTrace();
        }
        // 初始化加载图片
...
//        image3 = Toolkit.getDefaultToolkit().getImage(Panel.class.getResource("/TankGame/bomb_3.gif"));
    ...
    public void paint(Graphics graphics) {
        ...
        // 画出奖励
        if (myReward.isLive) {
            graphics.drawImage(imageStar, myReward.getX(), myReward.getY(), 20, 20, this);
        }
    ...
    private void drawTanks(int x, int y, Graphics graphics, int location, int type) {
        ...
        switch (location) {
            ...
            case 3:
                ...
                graphics.fillOval(x + 10, y + 5, 10, 10);
                // 最后画直线
                graphics.drawLine(x, y + 10, x + 15, y + 10);
                break;
        }
    ...
    // 画奖励的方法
    private void drawReward(int x, int y, Graphics graphics, int type) {
        /**
         * 画奖励的公共方法 x y 代表奖励的x轴 y轴位置 graphics 画笔 type 奖励的类型
         *
         */
        switch (type) {
            // 传入的类型是0
    ...
    private void hitEnemyTank() {
        ...
        for (int i = 0; i < this.myTank.shots.size(); i++) {
            ...
            if (myshot.isLive) {
                // 取出每个敌人坦克 判断子弹是否击中了它
                eneMyTank.stream().filter(em -> em.isLive).forEach(em -> {   //只要把主程序的类的实例对象放入方法中，然后调用对象的方法，改变对象的状态即可
...
/*    private boolean myTankStuck0(){
        isTouchOther(this.myTank, allTanks);
        if(this.myTank.isTouch)
    }*/


    // 判断是否有重叠发生
/*
    private void touchTank() {

        for (int i = 0; i < this.allTanks.size(); i++) {
            Tanks mytank = this.allTanks.get(i);
            allTanks.stream().filter(em -> em.isLive).forEach(em -> {   //只要把主程序的类的实例对象放入方法中，然后调用对象的方法，改变对象的状态即可
                this.isTouchOther(mytank, allTanks);
            });
...
*/
    ...
    public void run() {
        ...
        while (true) {
            ...
            this.myReward.rewardGot();

            if((times%100)==0){
                if (this.myReward.isLive == false) {
                    this.myReward.randomPosition();//刷新位置
                    this.myReward.isLive = true;
                }
            }
